...
class TestMicroChild(unittest.TestCase):
    ...
    @patch('src.cifar10.micro_child.tf.concat', return_value="concat")
    @patch('src.cifar10.micro_child.tf.nn.conv2d', return_value="conv2d")
    @patch('src.cifar10.micro_child.create_weight', return_value="create_weight")
    @patch('src.cifar10.micro_child.tf.nn.avg_pool', return_value="avg_pool")
    @patch('src.cifar10.micro_child.tf.pad', return_value=tf.constant(np.ones((4, 32, 32, 3))))
        ...
        with patch('src.cifar10.micro_child.Model.__init__', new=mock_init):
            ...
            with patch.object(mc, "_get_C", return_value=3) as getc:
                with patch.object(mc, "_get_strides", return_value="get_strides") as gets:
                    ...
                    conv2d.assert_called_with('avg_pool', 'create_weight', [1, 1, 1, 1], "VALID", data_format="NHWC")
        ...
        with patch('src.cifar10.micro_child.Model.__init__', new=mock_init):
            ...
            with patch.object(mc, "_get_C", return_value=3) as getc:
                with patch.object(mc, "_get_strides", return_value="get_strides") as gets:
                    ...
                    getc.assert_called_with('avg_pool')
                    create_weight.assert_called_with('w', [1, 1, 3, 12])
                    conv2d.assert_called_with('avg_pool', 'create_weight', [1, 1, 1, 1], "VALID", data_format="NCHW")
        ...
        with patch('src.cifar10.micro_child.Model.__init__', new=mock_init):
            ...
            with patch.object(mc, "_get_C", return_value=3) as getc:
                with patch.object(mc, "_get_strides", return_value="get_strides") as gets:
                    ...
                    conv2d.assert_called_with(None, "create_weight", [1, 1, 1, 1], 'SAME', data_format="NCHW")
    ...
    @patch('src.cifar10.micro_child.batch_norm', return_value="batch_norm")
    @patch('src.cifar10.micro_child.tf.nn.conv2d', return_value="conv2d")
    @patch('src.cifar10.micro_child.tf.nn.relu', return_value="relu")
    @patch('src.cifar10.micro_child.create_weight', return_value="create_weight")
    ...
    def test_model_nhwc_KNOWN_TO_FAIL(self, create_weight, conv2d, batch_norm):
        with patch('src.cifar10.micro_child.Model.__init__', new=mock_init):
            ...
            conv2d.assert_called_with({}, "create_weight", [1, 1, 1, 1], 'SAME', data_format="NHWC")
            batch_norm.assert_called_with("conv2d", True, data_format="NHWC")

    @patch('src.cifar10.micro_child.tf.matmul', return_value="matmul")
    @patch('src.cifar10.micro_child.tf.nn.dropout', return_value="dropout")
            ...
            with patch.object(mc, '_factorized_reduction', return_value="fr") as fr:
                with patch.object(mc, '_enas_layer', return_value="el") as el:
                    with patch.object(mc, '_get_C', return_value="get_c") as get_c:
                        ...
                        conv2d.assert_called_with({}, "create_weight", [1, 1, 1, 1], 'SAME', data_format="NCHW")
                        batch_norm.assert_called_with("conv2d", True, data_format="NCHW")
                        ...
                        matmul.assert_called_with('dropout', "create_weight")

    @patch('src.cifar10.micro_child.tf.compat.v1.layers')
    @patch('src.cifar10.micro_child.tf.matmul', return_value="matmul")
    @patch('src.cifar10.micro_child.tf.nn.dropout', return_value="dropout")
    ...
    @patch('src.cifar10.micro_child.create_weight', return_value="create_weight")
    @patch('src.cifar10.micro_child.print')
    def test_model_aux_heads(self, print, create_weight, conv2d, batch_norm, relu, gap, do, matmul, layers):
        layers.average_pooling2d = mock.MagicMock()
        with patch('src.cifar10.micro_child.Model.__init__', new=mock_init):
            mc = MicroChild({}, {}, num_epochs=310, drop_path_keep_prob=0.9)
            with patch.object(mc, '_factorized_reduction', return_value="fr") as fr:
                with patch.object(mc, '_enas_layer', return_value="el") as el:
                    with patch.object(mc, '_get_C', return_value="get_c") as get_c:
                        with patch.object(mc, "_get_HW", return_value="get_hw") as get_hw:
                            ...
                            mc._model({}, True)
                            create_weight.assert_any_call("w", [3, 3, 3, 72])
                            conv2d.assert_called_with("relu", "create_weight", [1, 1, 1, 1], 'SAME', data_format="NCHW")
                            ...
                            create_weight.assert_called_with("w", ["get_c", 10])
                            matmul.assert_called_with('dropout', "create_weight")
                            layers.average_pooling2d.assert_called_with("relu", [5, 5], [3, 3], "VALID", data_format="channels_last")
                            get_hw.assert_called_with("relu")

    @patch('src.cifar10.micro_child.batch_norm', return_value="batch_norm")
    @patch('src.cifar10.micro_child.tf.nn.separable_conv2d', return_value="s_conv2d")
    @patch('src.cifar10.micro_child.tf.nn.relu', return_value="relu")
    @patch('src.cifar10.micro_child.create_weight', return_value="create_weight")
    def test_fixed_conv(self, create_weight, relu, s_conv2d, batch_norm):
        with patch('src.cifar10.micro_child.Model.__init__', new=mock_init):
            ...
            with patch.object(mc, '_get_C', return_value="get_c") as get_c:
                ...
                s_conv2d.assert_called_with("relu", depthwise_filter="create_weight", pointwise_filter="create_weight", strides=[1, 1, 1, 1], padding="SAME", data_format="NHWC")
    ...
    @patch('src.cifar10.micro_child.tf.nn.relu', return_value="relu")
    @patch('src.cifar10.micro_child.create_weight', return_value="create_weight")
    def test_fixed_layer(self, create_weight, relu, conv2d, batch_norm, s_conv2d, layers):
        layers.average_pooling2d = mock.MagicMock(return_value="avg_pool2d")
        layers.max_pooling2d = mock.MagicMock(return_value="max_pool2d")
        with patch('src.cifar10.micro_child.Model.__init__', new=mock_init):
            mc = MicroChild({}, {}, num_epochs=310, drop_path_keep_prob=0.9)
                ...
                with patch.object(mc, '_get_C', return_value="get_c") as get_c:
                    with patch.object(mc, '_apply_drop_path', return_value="adp") as adp:
                        with patch.object(mc, '_fixed_combine', return_value="fc") as fc:
                            ...
                            conv2d.assert_called_with("relu", "create_weight", [1, 1, 1, 1], "SAME", data_format="NHWC")
                            batch_norm.assert_called_with("conv2d", True, data_format="NHWC")
                            s_conv2d.assert_called_with("relu", depthwise_filter="create_weight", pointwise_filter="create_weight", strides=[1, 1, 1, 1], padding="SAME", data_format="NHWC")
                            adp.assert_called_with('batch_norm', 0)
                            fc.assert_called()

    @patch('src.cifar10.micro_child.tf.stack', return_value=tf.constant(np.ndarray((1, 4, 32, 32, 3))))
    @patch('src.cifar10.micro_child.batch_norm', return_value="batch_norm")
    @patch('src.cifar10.micro_child.tf.nn.conv2d', return_value="conv2d")
    @patch('src.cifar10.micro_child.tf.nn.relu', return_value="relu")
    @patch('src.cifar10.micro_child.tf.reshape', return_value="reshape")
    @patch('src.cifar10.micro_child.create_weight', return_value="create_weight")
    @patch('src.cifar10.micro_child.tf.compat.v1.layers')
    def test_enas_cell(self, layers, create_weight, reshape, relu, conv2d, batch_norm, stack):
        layers.average_pooling2d = mock.MagicMock(return_value="avg_pool2d")
        with patch('src.cifar10.micro_child.Model.__init__', new=mock_init):
            mc = MicroChild({}, {}, num_epochs=310, drop_path_keep_prob=0.9)
            with patch.object(mc, '_get_C', return_value="get_c") as get_c:
                with patch.object(mc, '_enas_conv', return_value="ec") as ec:
                    ...
                    layers.average_pooling2d.assert_called_with(None, [3, 3], [1, 1], "SAME", data_format="channels_last")
                    get_c.assert_called_with(None)
                    create_weight.assert_called_with("w", [1, 'get_c' * 24])
                    reshape.assert_called_with('r', [1, 1, 'get_c', 24])
    ...
    @patch('src.cifar10.micro_child.tf.compat.v1.keras.initializers.ones', return_value="ones")
    @patch('src.cifar10.micro_child.tf.compat.v1.keras.initializers.zeros', return_value="zeros")
    @patch('src.cifar10.micro_child.tf.compat.v1.nn.fused_batch_norm', return_value="fbn")
    @patch('src.cifar10.micro_child.tf.nn.separable_conv2d', return_value="s_conv2d")
    @patch('src.cifar10.micro_child.tf.nn.relu', return_value="relu")
    @patch('src.cifar10.micro_child.tf.reshape', return_value="reshape")
    @patch('src.cifar10.micro_child.create_weight', return_value=tf.constant(np.ndarray((3, 3))))
    def test_enas_conv(self, create_weight, reshape, relu, s_conv, fbn, zeros, ones):
        with patch('src.cifar10.micro_child.Model.__init__', new=mock_init):
            ...
            with patch.object(mc, '_get_C', return_value="get_c") as get_c:
                ...
                fbn.assert_called()
                zeros.assert_called_with(dtype=tf.float32)
                ones.assert_called_with(dtype=tf.float32)

    @patch('src.cifar10.micro_child.tf.compat.v1.to_int32', return_value="to_int32")
    @patch('src.cifar10.micro_child.create_weight', return_value="create_weight")
    @patch('src.cifar10.micro_child.batch_norm', return_value="batch_norm")
    @patch('src.cifar10.micro_child.tf.nn.conv2d', return_value="conv2d")
    @patch('src.cifar10.micro_child.tf.nn.relu', return_value="relu")
    @patch('src.cifar10.micro_child.tf.reshape', return_value="reshape")
    @patch('src.cifar10.micro_child.tf.transpose', return_value="transpose")
    @patch('src.cifar10.micro_child.tf.gather', return_value="gather")
    @patch('src.cifar10.micro_child.tf.stack', return_value=tf.constant(np.ndarray((2, 3, 32, 32, 3))))
    ...
    @patch('src.cifar10.micro_child.tf.reduce_sum', return_value="reduce_sum")
    @patch('src.cifar10.micro_child.tf.equal', return_value="equal")
    @patch('src.cifar10.micro_child.tf.compat.v1.to_int32', return_value="to_int32")
    @patch('src.cifar10.micro_child.tf.argmax', return_value="argmax")
    @patch('src.cifar10.micro_child.tf.reduce_mean', return_value=1.0)
    @patch('src.cifar10.micro_child.tf.nn.sparse_softmax_cross_entropy_with_logits', return_value="sscewl")
    @patch('src.cifar10.micro_child.print')
    ...
    @patch('src.cifar10.micro_child.tf.map_fn', return_value="map_fn")
    @patch('src.cifar10.micro_child.tf.reduce_sum', return_value="reduce_sum")
    @patch('src.cifar10.micro_child.tf.equal', return_value="equal")
    @patch('src.cifar10.micro_child.tf.compat.v1.to_int32', return_value="to_int32")
    @patch('src.cifar10.micro_child.tf.argmax', return_value="argmax")
    @patch('src.cifar10.micro_child.tf.compat.v1.train.shuffle_batch', return_value=(tf.constant(np.ndarray((2, 32, 32, 3))), "y_valid_shuffle"))
    @patch('src.cifar10.micro_child.print')
