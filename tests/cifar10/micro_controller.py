...
class TestMicroController(unittest.TestCase):
    @patch('src.cifar10.micro_controller.tf.reduce_sum', return_value="reduce_sum")
    @patch('src.cifar10.micro_controller.tf.reshape', return_value="reshape")
    @patch('src.cifar10.micro_controller.tf.matmul', return_value="matmul")
    @patch('src.cifar10.micro_controller.tf.TensorArray')
    @patch('src.cifar10.micro_controller.stack_lstm', return_value=([1], [2]))
    @patch('src.cifar10.micro_controller.tf.compat.v1.get_variable', return_value="gv")
    @patch('src.cifar10.micro_controller.tf.random_uniform_initializer', return_value="rui")
    @patch('src.cifar10.micro_controller.print')
    def test_constructor(self, print, rui, gv, stack_lstm, tensor_array, matmul, reshape, reduce_sum):
        ...
        with patch('src.cifar10.micro_controller.tf.while_loop', return_value=mock_tensor_array) as while_loop:
    ...
    @patch('src.cifar10.micro_controller.tf.control_dependencies')
    @patch('src.cifar10.micro_controller.tf.compat.v1.to_float', return_value=1.0)
    @patch('src.cifar10.micro_controller.tf.reduce_sum', return_value=1.0)
    @patch('src.cifar10.micro_controller.tf.reshape', return_value="reshape")
    @patch('src.cifar10.micro_controller.tf.matmul', return_value="matmul")
    @patch('src.cifar10.micro_controller.tf.TensorArray')
    @patch('src.cifar10.micro_controller.stack_lstm', return_value=([1], [2]))
    ...
    @patch('src.cifar10.micro_controller.tf.compat.v1.assign_sub', return_value="assign_sub")
    @patch('src.cifar10.micro_controller.print')
    def test_build_trainer(self, print, assign_sub, get_train_ops, rui, get_variable, stack_lstm, tensor_array, matmul, reshape, reduce_sum, to_float, cd):
        mock_tensor_array = mock.MagicMock()
        mock_tensor_array.write = mock.MagicMock(return_value=mock_tensor_array)
        mock_tensor_array.stack = mock.MagicMock(return_value="stack")
        tensor_array.return_value = mock_tensor_array
