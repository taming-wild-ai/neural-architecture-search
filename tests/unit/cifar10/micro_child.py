...
class TestMicroChild(unittest.TestCase):
    ...
    def test_model_nchw(self, print, create_weight, conv2d, batch_norm, relu, do, matmul):
        with patch('src.cifar10.micro_child.Child.__init__', new=mock_init_nchw):
            with tf.Graph().as_default():
                mc = MicroChild({}, {})
    ...
    @patch('src.cifar10.micro_child.fw.create_weight', return_value="fw.create_weight")
    @patch('src.cifar10.micro_child.print')
    def test_model_aux_heads(self, print, create_weight, conv2d, batch_norm, relu, do, matmul, avg_pool2d):
        with patch('src.cifar10.micro_child.Child.__init__', new=mock_init_nchw):
            with tf.Graph().as_default():
                mc = MicroChild({}, {})
