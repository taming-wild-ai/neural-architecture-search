...
class MicroChild(Model):
  ...
  def _apply_drop_path(self, x, layer_id):
    ...
    step_ratio = tf.compat.v1.to_float(self.global_step + 1) / tf.compat.v1.to_float(self.num_train_steps)
  ...
  def _maybe_calibrate_size(self, layers, out_filters, is_training):
    ...
    with tf.compat.v1.variable_scope("calibrate"):
      x = layers[0]
      if hw[0] != hw[1]:
        assert hw[0] == 2 * hw[1]
        with tf.compat.v1.variable_scope("pool_x"):
          x = tf.nn.relu(x)
      ...
      y = layers[1]
      if c[1] != out_filters:
        with tf.compat.v1.variable_scope("pool_y"):
      ...
      for layer_id in range(self.num_layers + 2):
        ...
        if (self.use_aux_heads and
          ...
          with tf.compat.v1.variable_scope("aux_head"):
            ...
            aux_logits = tf.compat.v1.layers.average_pooling2d(
          ...
          aux_head_variables = [
            var for var in tf.compat.v1.trainable_variables() if (
  ...
  def _fixed_conv(self, x, f_size, out_filters, stride, is_training,
    ...
    for conv_id in range(stack_convs):
      ...
      with tf.compat.v1.variable_scope("sep_conv_{}".format(conv_id)):
  ...
  def _fixed_combine(self, layers, used, out_filters, is_training,
    ...
    with tf.compat.v1.variable_scope("final_combine"):
      for i, layer in enumerate(layers):
        if used[i] == 0:
          hw = self._get_HW(layer)
          if hw > out_hw:
            assert hw == out_hw * 2, ("i_hw={0} != {1}=o_hw".format(hw, out_hw))
            with tf.compat.v1.variable_scope("calibrate_{0}".format(i)):
  ...
  def _fixed_layer(self, layer_id, prev_layers, arc, out_filters, stride,
    ...
    with tf.compat.v1.variable_scope("layer_base"):
      x = layers[1]
      inp_c = self._get_C(x)
      w = create_weight("w", [1, 1, inp_c, out_filters])
      x = tf.nn.relu(x)
    ...
    for cell_id in range(self.num_cells):
      with tf.compat.v1.variable_scope("cell_{}".format(cell_id)):
        x_id = arc[4 * cell_id]
        used[x_id] += 1
        x_op = arc[4 * cell_id + 1]
        x = layers[x_id]
        x_stride = stride if x_id in [0, 1] else 1
        with tf.compat.v1.variable_scope("x_conv"):
          if x_op in [0, 1]:
            f_size = f_sizes[x_op]
        ...
        with tf.compat.v1.variable_scope("y_conv"):
          if y_op in [0, 1]:
            f_size = f_sizes[y_op]
            ...
            inp_c = self._get_C(y)
            if y_op == 2:
              y = tf.compat.v1.layers.average_pooling2d(
                y, [3, 3], [y_stride, y_stride], "SAME",
                data_format=self.actual_data_format)
            else:
              y = tf.compat.v1.layers.max_pooling2d(
                y, [3, 3], [y_stride, y_stride], "SAME",
                data_format=self.actual_data_format)
            if inp_c != out_filters:
              w = create_weight("w", [1, 1, inp_c, out_filters])
  ...
  def _enas_conv(self, x, curr_cell, prev_cell, filter_size, out_filters,
    ...
    with tf.compat.v1.variable_scope("conv_{0}x{0}".format(filter_size)):
      ...
      for conv_id in range(stack_conv):
        with tf.compat.v1.variable_scope("stack_{0}".format(conv_id)):
  ...
  def _enas_layer(self, layer_id, prev_layers, arc, out_filters):
    ...
    for cell_id in range(self.num_cells):
      ...
      with tf.compat.v1.variable_scope("cell_{0}".format(cell_id)):
